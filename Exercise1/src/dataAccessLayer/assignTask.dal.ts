...
const checkUserExists = async (assignToUserEmail: string): Promise<iUser> => {
    const isUserExists: iUser = await User.findOne({ email: assignToUserEmail });
    return isUserExists;
}

const assignTaskToExistingUser = async (newAssignTask): Promise<iTask> => {
    const newTaskObj: iTask = new Task(newAssignTask);
    const taskFromDb: iTask = await newTaskObj.save();
    return taskFromDb;
}

const assignTaskToNonExistingUser = async (newAssignTask): Promise<iAssignTask> => {
...
export {
    checkUserExists,
    assignTaskToExistingUser,
    assignTaskToNonExistingUser
}