...
const isUserAlreadyExists = async (userEmail: string): Promise<iUser> => {
    const isUserAlreadyExist: iUser = await User.findOne({ email: userEmail });
    return isUserAlreadyExist;
}

const createNewUser = async (userToRegister): Promise<iUser> => {
    const user: iUser = new User(userToRegister);
    const userFromDb: iUser = await user.save();
    return userFromDb;
}

const assignTaskToNewUser = async (userFromDb: iUser, userEmail: string): Promise<iAssignTask[]> => {
...
const checkLoginCredientials = async (userEmail: string, userPassword: string): Promise<iUser> => {
